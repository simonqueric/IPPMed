...
class CustomDataset(Dataset):
    def __init__(self, data_dir, predict=False):
        ...
        self.file_list = os.listdir(self.scan_volume_dir)
    ...
    def __getitem__(self, idx):
        ...
        scan_volume_path = os.path.join(self.scan_volume_dir, self.file_list[idx])
        base_filename = self.file_list[idx].split('_vol.nii.gz')[0]
        if not self.predict:
            lung_seg_path = os.path.join(self.lung_seg_dir, base_filename + '_lungseg.nii.gz')
